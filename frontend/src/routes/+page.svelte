...
<main class="app-shell">
  <section class="phone-frame">
    ...
    {#if screen === 'register'}<section class="card register-page"><div class="section-title"><span>✦</span><h2>{copy.registerTitle}</h2></div><p>{copy.registerBody}</p><button class="google-action" on:click={() => (screen = 'terms')}>G {copy.googleLogin}</button><div class="split"><input placeholder="Email" /><input placeholder="Nama" /></div><button class="primary-action" on:click={() => (screen = 'hermes')}>{copy.back}</button></section>{/if}
