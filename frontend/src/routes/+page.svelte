<script lang="ts">
  ...
  const t = {
    id: {
      badge: 'Game Astrologi', title: 'Hermex Fun',
      subtitle: 'Hermex Fun adalah game astrologi open-source untuk belajar natal chart, menyimpan history guest, dan membaca refleksi diri lewat Hermes AI.',
  ...
  onMount(() => {
    const savedLang = localStorage.getItem('hermex_lang');
  ...
  function toggleLang() {
    lang = lang === 'id' ? 'en' : 'id';
    if (typeof localStorage !== 'undefined') localStorage.setItem('hermex_lang', lang);
  }
  ...
  function chooseCity(city: CityOption) { selectedCity = city; birth_place = city.label; cityQuery = city.label; cityPickerOpen = false; error = ''; }
  function hasValidCitySelection() {
    return Boolean(selectedCity && selectedCity.label.trim().toLowerCase() === cityQuery.trim().toLowerCase());
  }
  function onCityInput(value: string) {
    ...
    if (!selectedCity && formStep === 1) error = '';
  }
  ...
  async function runAnalysis() {
    if (!hasValidCitySelection()) {
      formStep = 1;
      error = lang === 'id' ? 'Pilih tempat lahir dari daftar kota dulu.' : 'Choose a birth place from the city list first.';
      return;
    }
...
<main class="app-shell">
  <section class="phone-frame">
    ...
    {#if screen === 'home'}
      <section class="hero-card"><div><span>{copy.badge}</span><h1>{copy.title}</h1><p>{copy.subtitle}</p><a class="repo-link" href="https://github.com/wauputr4/hermex" target="_blank" rel="noreferrer">↗ {copy.repoLink}</a></div><svg viewBox="0 0 220 220"><circle cx="110" cy="110" r="82"/><circle cx="110" cy="110" r="50"/><path d="M138 47a31 31 0 1 0 0 62 38 38 0 1 1 0-62z"/><circle cx="49" cy="83" r="13"/><circle cx="171" cy="148" r="16"/><path d="M101 128l8 18 18 8-18 8-8 18-8-18-18-8 18-8 8-18z"/></svg></section>
      ...
      {#if latestSkyPost}<section class="latest-sky"><small>{lang === 'id' ? 'Berita Langit Terbaru' : 'Latest Sky News'}</small><h2>{latestSkyPost.title}</h2><p>{latestSkyPost.summary}</p><a href={`/berita-langit/${latestSkyPost.slug}`}>{lang === 'id' ? 'Baca artikel' : 'Read article'}</a></section>{/if}
      <section class="home-grid single"><article class="card form-card quest-form"><div class="section-title"><span>01</span><h2>{copy.playerProfile}</h2></div>
        <div class="quest-steps">{#each formSteps as step, index}<button class:active={formStep === index} disabled={index === 2 && !hasValidCitySelection()} on:click={() => { if (index < 2 || hasValidCitySelection()) formStep = index; }}><span>{index + 1}</span>{step}</button>{/each}</div>
        ...
        <div class="route-actions">{#if formStep > 0}<button class="soft-action" on:click={() => (formStep = Math.max(0, formStep - 1))}>{copy.back}</button>{/if}{#if formStep < 2}<button class="primary-action" disabled={formStep === 1 && !hasValidCitySelection()} on:click={() => { if (formStep === 1 && !hasValidCitySelection()) return; formStep = Math.min(2, formStep + 1); }}>{lang === 'id' ? 'Lanjut quest' : 'Next quest'}</button>{:else}<button class="primary-action" on:click={runAnalysis} disabled={loading}>✦ {loading ? copy.analyzing : copy.analyze}</button>{/if}</div>{#if error}<p class="error">{error}</p>{/if}
      </article></section>
    {/if}
    ...
    {#if screen === 'connect'}<section class="card connect-page"><div class="section-title"><span>↗</span><h2>{copy.connect}</h2></div><p>{lang === 'id' ? 'Publikasikan profil astrologi seperti shortlink bio. Email dan username wajib agar link publik tidak duplikat.' : 'Publish an astrology profile like a bio shortlink. Email and username are required so public links stay unique.'}</p><div class="split"><label>{copy.email}<input type="email" bind:value={email} placeholder="you@example.com" /></label><label>{copy.username}<input bind:value={usernameInput} placeholder="wauputra" on:input={() => (usernameInput = safeUsername(usernameInput))} /><small>{copy.usernameHint}</small></label></div><button class="share-action" on:click={shareProfile}>↗ {publishLoading ? copy.analyzing : copy.publishProfile}</button>{#if publicProfileStatus}<small>{publicProfileStatus}</small>{/if}{#if shareStatus}<small>{shareStatus}</small>{/if}{#if shareLink}<input class="share-link" readonly value={shareLink} on:focus={(event) => event.currentTarget.select()} />{/if}<h3>{lang === 'id' ? 'Connect publik' : 'Public Connect'}</h3>{#if publicProfiles.length}<div class="connect-grid">{#each publicProfiles as item}<button on:click={() => openPublicProfile(item.username)}><strong>@{item.username}</strong><span>{item.display_name || 'Guest'} · {item.profile?.traits?.dominant_element}</span><small>{item.latest_interpretation ? publicSummary(item.latest_interpretation.interpretation?.summary).slice(0, 90) : 'Natal chart ready'}</small></button>{/each}</div>{:else}<p>{lang === 'id' ? 'Belum ada profil publik. Jadilah yang pertama.' : 'No public profiles yet. Be the first.'}</p>{/if}</section>{/if}
    {#if screen === 'public-profile' && publicProfile}<section class="card public-page"><div class="section-title"><span>↗</span><h2>@{publicProfile.username}</h2></div><div class="public-hero">{@render Icon(0)}<div><h3>{publicProfile.display_name || 'Hermex Guest'}</h3><p>{publicProfile.bio || (lang === 'id' ? 'Profil astrologi publik dari Hermex Fun.' : 'Public astrology profile from Hermex Fun.')}</p><div class="chips compact"><span>{publicProfile.profile?.traits?.dominant_element}</span>{#each (publicProfile.profile?.traits?.interests ?? []).slice(0, 3) as item}<span>{item}</span>{/each}</div></div></div>{#if publicProfile.latest_interpretation}<div class="summary-card mini">{@render Icon(1)}<p>{publicSummary(publicProfile.latest_interpretation.interpretation?.summary)}</p></div>{/if}<button class="soft-action oracle-button" on:click={() => { profile = publicProfile.profile; interpretation = publicProfile.latest_interpretation; screen = interpretation ? 'hermes' : 'profile'; }}>{lang === 'id' ? 'Buka detail astrologi' : 'Open astrology detail'}</button></section>{/if}
    {#if screen === 'sky-news'}<section class="card sky-page"><div class="section-title"><span>☾</span><h2>{copy.skyNews}</h2></div><p>{lang === 'id' ? 'Blog ringan tentang simbol langit dan cara membacanya sebagai refleksi peristiwa di bumi, bukan klaim sebab-akibat mutlak.' : 'A light blog about sky symbols and how to read them as earthly reflection, not deterministic causality.'}</p>{#if latestSkyPost}<article class="latest-sky in-page"><small>{latestSkyPost.tag}</small><h2>{latestSkyPost.title}</h2><p>{latestSkyPost.summary}</p><a href={`/berita-langit/${latestSkyPost.slug}`}>{lang === 'id' ? 'Baca artikel terbaru' : 'Read latest article'}</a></article>{/if}<div class="calendar-card"><h3>{lang === 'id' ? 'Kalender Astrologi' : 'Astrology Calendar'}</h3>{#if skyCalendar.length}<div class="calendar-list">{#each skyCalendar as event}<article><time>{event.event_date}</time><strong>{event.title}</strong><span>{event.tag}</span><p>{event.summary}</p></article>{/each}</div>{:else}<p>{lang === 'id' ? 'Belum ada event kalender.' : 'No calendar events yet.'}</p>{/if}</div><div class="sky-list">{#each skyPosts as post}<article>{@render Icon(3)}<div><small>{post.tag}</small><h3>{post.title}</h3><p>{post.summary}</p><a href={`/berita-langit/${post.slug}`}>{lang === 'id' ? 'Buka artikel' : 'Open article'}</a></div></article>{/each}</div></section>{/if}
...
@media (max-width: 720px) {
  ...
  .quest-steps { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; overflow: visible; padding: 0; }
  .quest-steps button { min-width: 0; display: flex; align-items: center; justify-content: center; gap: 6px; text-align: center; line-height: 1.15; font-size: .82rem; }
  .quest-steps span { margin-right: 0; flex: 0 0 auto; }
  ...
  .city-menu { position: static; margin-top: 8px; max-height: 260px; border-radius: 20px; padding: 8px; z-index: 12; }
  .city-menu button { padding: 12px; }
  .city-menu small { line-height: 1.25; }
